...
import {  Iuser, IuserValidate } from "../../@types/user";
import { IAddress, IImage, IName, IRegiserInputs } from "../../@types/global";
import validation from "../validation";


const registerSchema = Joi.object<IuserValidate>({
  ...
  phoneNumber: Joi.string().min(9).max(11).pattern(phonePattern).required().messages({
  ...
  password: Joi.string()
    ...
    .messages({
      "string.pattern.base": "password should be at least one special charcter and one upper case charcter",
      "string.empty":
        "password should be at least 7 charcters",
    ...
    .max(20)
    .required(),
    passworConfirmation:Joi.string().valid(Joi.ref('password'))
    .required()
    .messages({
